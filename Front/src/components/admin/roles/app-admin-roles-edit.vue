  ...
  >
    <template v-slot:activator="{ on, attrs }">
      <v-btn
        ...
        small
        icon
        @click="form.inputs.description = rol.description"
      >
    ...
    <v-form ref="form" v-model="form.valid" lazy-validation @submit="save">
      <v-card>
        <v-toolbar color="teal" dark>Modificar un rol</v-toolbar>
        <v-card-text>
          <v-text-field :value="rol.id" disabled label="Id del rol">
          </v-text-field>
          <v-text-field
...
export default {
  ...
  methods: {
    async save(event) {
      ...
      this.form.inputs.id = this.rol.id;
      ...
      try {
        const res = await fetch("/api/Roles/"+this.form.inputs.id, requestOptions);
        if (res.status >= 200 && res.status <= 299) {
          this.Log.add("Role", "U", this.form.inputs.id, this.form.inputs);
          this.$swal.fire({
